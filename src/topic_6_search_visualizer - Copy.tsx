...
type StepLog = {
    ...
    events: string[]; // Detailed logs like "Node B Updated", "Node C Pruned"
};
...
const GraphVisualizer = () => {
    ...
    const runAlgorithm = async () => {
        ...
        if (algorithm === 'MiniMax') {
            ...
            const minimax = async (nodeId: string, depth: number, isMax: boolean, path: string[]): Promise<MinimaxResult> => {
                ...
                for (const neighborId of neighbors) {
                    if (path.includes(neighborId)) continue; // Cycle check
        ...
        // Data Structures
        // Note: For BFS/DFS we still use array as Queue/Stack.
        // For UCS/A* we use array but sort it every time to simulate Priority Queue.
        let openSet: { id: string; g: number; f: number; path: string[] }[] = [];
        ...
        let stepCount = 0; // Will be displayed as 0-indexed log
        let extendedCount = 0;
        let enqueuedCount = 1;
        ...
        setStepLogs([{
            step: 0,
            queue: [{ path: [startNode], cost: 0, heuristic: startH, total: startH }],
            extendedNode: null,
            extendedList: [],
            extendedCount: 0,
            enqueuedCount: 1,
        ...
        while (openSet.length > 0) {
            ...
            // 1. Sort Open Set (Priority Queue)
            if (algorithm === 'UCS') {
                openSet.sort((a, b) => {
            ...
            let current;
            if (algorithm === 'DFS') {
                // DFS: Pop from End (Stack)
                current = openSet.pop()!;
            } else {
                current = openSet.shift()!; // BFS, UCS, AStar
            }

            // Lazy Removal Check (For A*)
            // If current node is already visited with a lower/equal cost, we skip extending it.
            // but we LOG it as "Not Extended" for A* to match slides.
            let isPruned = false;
            let pruneReason = "";

            if (!treeSearch && algorithm === 'AStar') {
                const previousCost = closedSet.get(current.id);
                if (previousCost !== undefined && previousCost <= current.g) {
                    isPruned = true;
                    // Format matching slide: "Not Extended due to Extended List" or similar
                    pruneReason = `Node ${current.id} Not Extended (Visited with cost ${previousCost} <= ${current.g})`;
                }
            }
            ...
            if (isPruned) {
                ...
                setStepLogs(prev => [...prev, {
                    step: stepCount,
                    queue: openSet.map(n => ({ path: n.path, cost: n.g, heuristic: heuristics[n.id] || 0, total: n.f })),
                    extendedNode: null,
            ...
            if (current.id === goalNode) {
                setStepLogs(prev => [...prev, {
                    ...
                    extendedCount: extendedCount,
                    enqueuedCount: enqueuedCount,
                    events: [`Goal ${goalNode} Reached! Cost: ${current.g}`]
                }]);
            ...
            // Get Neighbors
            // FIX: Ensure Undirected graph neighbors are correctly found
            let neighbors: { to: string, weight: number }[] = [];

            if (isDirected) {
                neighbors = edges.filter(e => e.from === current.id).map(e => ({ to: e.to, weight: e.weight }));
            } else {
                // Undirected: Find all edges connected to current
                const connected = edges.filter(e => e.from === current.id || e.to === current.id);
                neighbors = connected.map(e => {
                    if (e.from === current.id) return { to: e.to, weight: e.weight };
                    return { to: e.from, weight: e.weight }; // Swap
                });
            }

            // Sort Neighbors
            if (algorithm === 'DFS') {
                neighbors.sort((a, b) => b.to.localeCompare(a.to)); // Z-A (so A is popped first)
            } else if (algorithm === 'BFS') {
                neighbors.sort((a, b) => a.to.localeCompare(b.to)); // A-Z
            } else {
                // A* / UCS: Weight then Alpha?? Or just Alpha?
                // Usually just Alpha is consistent for tie-breaking generation.
                // But typically we iterate neighbors in alpha order.
            ...
            for (const neighbor of neighbors) {
                ...
                const h = heuristics[neighbor.to] || 0;
                const f = tentativeG + h;
                const newPath = [...current.path, neighbor.to];

                // Check Cycles (Tree Search)
                if (treeSearch && current.path.includes(neighbor.to)) {
                    continue; // Skip cycle in current path
                }

                if (!treeSearch) {
                    // Check Closed Set
                    const closedG = closedSet.get(neighbor.to);
                    if (closedG !== undefined) {
                        if (closedG <= tentativeG) {
                            stepEvents.push(`Node ${neighbor.to} Not Extended (Visited with cost ${closedG} <= ${tentativeG})`);
                            continue;
                        } else {
                            // Re-opening (Rare in consistent heuristics, but possible)
                            // Remove from Closed Set? 
                            // Actually pure A* says: if in closed with higher cost, move to open.
                            closedSet.delete(neighbor.to);
                            stepEvents.push(`Node ${neighbor.to} Re-opened (Better path found)`);
                        }
                    }
                }

                // Check Open Set (Frontier)
                const openIndex = openSet.findIndex(n => n.id === neighbor.to);

                if (openIndex !== -1) {
                    // Already in Queue
                    const existing = openSet[openIndex];
                    if (treeSearch) {
                        // Tree Search: Always add duplicates
                        openSet.push({ id: neighbor.to, g: tentativeG, f, path: newPath });
                        enqueuedCount++;
                        stepEvents.push(`Node ${neighbor.to} Enqueued (Duplicate)`);
                    } else {
                        // Graph Search: Update if better
                        if (tentativeG < existing.g) {
                            openSet[openIndex] = { id: neighbor.to, g: tentativeG, f, path: newPath };
                            stepEvents.push(`Node ${neighbor.to} Entry Updated (Cost ${existing.g} -> ${tentativeG})`);
                        } else {
                            stepEvents.push(`Node ${neighbor.to} Not Extended (Queue has better/equal cost ${existing.g})`);
                        }
                    }
                } else {
                    // New Node
                    openSet.push({ id: neighbor.to, g: tentativeG, f, path: newPath });
                    enqueuedCount++;
                    stepEvents.push(`Node ${neighbor.to} Enqueued`);
                }
            }

            // Log Step (After processing all neighbors)
            // Determine "Snapshot" to show. Usually we show the queue state at the END of the step (ready for next).
            // For DFS, reverse queue for display
            const queueDisplay = algorithm === 'DFS' ? [...openSet].reverse() : [...openSet];

            setStepLogs(prev => [...prev, {
                step: stepCount,
                queue: queueDisplay.map(n => ({ path: n.path, cost: n.g, heuristic: heuristics[n.id] || 0, total: n.f })),
                extendedNode: current.id,
                extendedList: Array.from(closedSet.keys()),
                extendedCount: extendedCount,
                enqueuedCount: enqueuedCount,
                            ...
                            <table className="w-full text-xs text-left border-collapse">
                                <thead className="bg-slate-50 text-slate-500 sticky top-0">
                                    <tr>
                                        ...
                                        <th className="p-2 border-b border-slate-200 w-1/3">Enqueued Paths</th>
                                        <th className="p-2 border-b border-slate-200">Events</th>
                                        <th className="p-2 border-b border-slate-200 w-20 text-center">Extended</th>
                                        <th className="p-2 border-b border-slate-200 w-20 text-center">Enqueued</th>
                                    </tr>
                                </thead>
                                            ...
                                            <td className="p-2 text-xs text-slate-600 align-top">
                                                {log.events && log.events.length > 0 ? (
                                                    <ul className="list-disc list-inside space-y-0.5">
                                                        {log.events.map((e, i) => (
                                                            <li key={i} className={e.includes("Updated") ? "text-blue-600 font-bold" : (e.includes("Not Extended") ? "text-slate-400" : "")}>
                                                                {e}
                                                            </li>
                                                        ))}
                                                    </ul>
